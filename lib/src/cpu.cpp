...
void CPU::fetchInstruction()
{
    ...
    {
        std::cerr << "Attempted to fetch NULL instruction: " << m_curInst << std::endl;
    }
...
void CPU::fetchData()
{
    ...
    switch(m_curInst.mode){ //Cycle only when reading from bus, and inc only when reading from bus
        case AM_IMP: break;
        ...
        case AM_R_MR: //Register holds address on bus we want to read for data
            {
                ...
                m_fetchData = m_bus->busRead(addr); //We don't increase because we're pulling a direct address
        ...
        default:
            if(m_curInst.mode > 20){
                std::cerr << "Unknown addressing mode: " << (uint8_t)m_curInst.mode << std::endl;
                throw std::out_of_range("Attempting to access non existing address mode");
            }
            else
            {
                std::cerr << "Address Mode " << (uint8_t)m_curInst.mode << " Not Implemented" << std::endl;
            }
            break;
...
uint16_t CPU::readReg(RegType rt)
{
    switch(rt){
        ...
        case RT_AF: return htons(*(uint16_t*)&m_regs.a);//combine A and F registers so we access register f 
                                                        //because we're taking a 16 bit value pointer (memory goes beyond 8 bit a)
                                                        //using htons because host to network we're reading reg
                                                        //doesn't really matter if we use htons or ntohs
                                                        //A and F are next to eachother in struct so accessing a this way will give f
        case RT_BC: return htons(*(uint16_t*)&m_regs.b);
        case RT_DE: return htons(*(uint16_t*)&m_regs.d);
        case RT_HL: return htons(*(uint16_t*)&m_regs.h);

        case RT_PC: return m_regs.pc;
        case RT_SP: return m_regs.sp;
...
{
    switch(rt){
        case RT_A: m_regs.a = val & 0xFF; break; //Taking 16 bit but only first byte & with 0xFF ignores 2nd byte;
        case RT_F: m_regs.f = val & 0xFF; break;
        case RT_B: m_regs.b = val & 0xFF; break;
        case RT_C: m_regs.c = val & 0xFF; break;
        case RT_D: m_regs.d = val & 0xFF; break;
        case RT_E: m_regs.e = val & 0xFF; break;
        case RT_H: m_regs.h = val & 0xFF; break;
        case RT_L: m_regs.l = val & 0xFF; break;

        case RT_AF: *(uint16_t*)&m_regs.a = ntohs(val); break;//ntohs because cpu is receiving data
        case RT_BC: *(uint16_t*)&m_regs.b = ntohs(val); break;
        case RT_DE: *(uint16_t*)&m_regs.d = ntohs(val); break;
        case RT_HL: *(uint16_t*)&m_regs.h = ntohs(val); break;

        case RT_PC: m_regs.pc = val; break;
        case RT_SP: m_regs.pc = val; break;
        default: break;
    }
}